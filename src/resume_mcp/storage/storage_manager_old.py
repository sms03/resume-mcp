...
class StorageManager:
    ...
    def save_resume(self, resume_data: ResumeData, filename: Optional[str] = None) -> str:
        ...
        content_str = resume_data.model_dump_json()
    ...
    def save_job_description(self, job_data: JobPosting) -> str:
        ...
        """
        # Generate content hash to avoid duplicates
        content_str = job_data.model_dump_json()
        content_hash = hashlib.sha256(content_str.encode()).hexdigest()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            job_id = job_data.id if hasattr(job_data, 'id') and job_data.id else str(uuid.uuid4())
    ...
    def save_match_result(self, match_result: MatchResult) -> str:
        ...
        content_str = match_result.model_dump_json()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """, (
                ...
                match_result.overall_match.match_level,
                content_str
            ))
    ...
    def get_resume(self, resume_id: str) -> Optional[ResumeData]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if result:
                return ResumeData.model_validate_json(result[0])
            return None
    ...
    def get_job_description(self, job_id: str) -> Optional[JobPosting]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if result:
                return JobPosting.model_validate_json(result[0])
            return None
    ...
    def get_match_result(self, match_id: str) -> Optional[MatchResult]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if result:
                return MatchResult.model_validate_json(result[0])
            return None
